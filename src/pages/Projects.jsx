...
const mediumMap = { 'Short Film': 'short-film', 'Commercial': 'commercial', 'Reel': 'reel' }

function WebCard({ project, cardRef, onMouseEnter, onMouseMove, onMouseLeave }) {
...
function VideoCard({ project, cardRef, onMouseEnter, onMouseMove, onMouseLeave }) {
  const gifEntry = cardGifMap[project.slug]
  return (
    <Link to={`/projects/${project.slug}`} className="project-card-link">
      <div
        ref={cardRef}
        ...
        onMouseLeave={onMouseLeave}
      >
        <div className="project-image-container project-image-container--video">
          <img src={gifEntry ? gifEntry.static : project.image} alt={project.title} className="project-image" />
          {gifEntry && <img src={gifEntry.gif} alt="" className="project-image--gif" />}
          <div className="project-overlay project-overlay--video">
          ...
          <span className="video-type-label">{mediumLabel[project.medium] || project.medium}</span>
        </div>
        <div className="project-content">
          <h3 className="project-title">{project.title}</h3>
...
export default function ProjectsPage() {
  ...
  const primaryFilters = ['All', 'Web & Apps', 'Film & Video']
  const videoSubFilters = ['All', 'Commercial', 'Reel', 'Short Film']
  const ownershipFilters = ['All', 'Personal', 'Client']

  const filteredProjects = projectsData.filter(project => {
    if (activePrimary === 'Web & Apps' && project.type !== 'web') return false
    if (activePrimary === 'Film & Video' && project.type !== 'video') return false
    if (activePrimary === 'Film & Video' && activeSubFilter !== 'All' && project.medium !== mediumMap[activeSubFilter]) return false
    if (activeOwnership !== 'All' && project.category !== activeOwnership) return false
    return true
  })
  ...
  const handleMouseEnter = (index) => {
    ...
    if (el) {
      ...
      el.style.transition = 'box-shadow 0.3s ease, border-color 0.25s ease'
    }
  }
      ...
      <section className="projects-section">
        <div className="projects-wrapper">
          ...
          <p className="projects-intro">
            Software, film, and creative work — personal projects and client collaborations.
          </p>
          ...
          <div className="filters-bar">
            <div className="primary-filters">
              {primaryFilters.map(f => (
                <button
                  key={f}
                  className={`primary-filter${activePrimary === f ? ' active' : ''}`}
                  onClick={() => { setActivePrimary(f); setActiveSubFilter('All') }}
                >
                  {f}
                </button>
              ))}
            ...
            <div className="ownership-toggle">
              {ownershipFilters.map(f => (
                <button
                  key={f}
                  className={`ownership-btn${activeOwnership === f ? ' active' : ''}`}
                  onClick={() => setActiveOwnership(f)}
                >
                  {f}
                </button>
              ))}
          ...
          {activePrimary === 'Film & Video' && (
            <div className="sub-filters">
              {videoSubFilters.map(f => (
                <button
                  key={f}
                  className={`sub-filter${activeSubFilter === f ? ' active' : ''}`}
